      ...
      <<~JS.html_safe
        document.addEventListener("DOMContentLoaded", function () {
          const config = {
            ...
            signOutUrl: "#{Rails.application.routes.url_helpers.destroy_user_session_path}",
            signInUrl: "#{Rails.application.routes.url_helpers.new_user_session_path}"
          };
          ...
          function createTimeoutModal() {
            ...
            modal.style.cssText = "display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); z-index: 1000;";

            const modalContent = document.createElement("div");
            modalContent.classList.add("govuk-panel", "govuk-panel--confirmation");
            modalContent.style.cssText = "position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); padding: 20px; text-align: center; background-color: #fff; outline: none;";
